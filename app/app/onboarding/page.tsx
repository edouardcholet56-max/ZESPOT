...
export default function OnboardingPage() {
  ...
  const requestGeo = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        setGeoGranted(true)
        // Reverse geocode via API Google
        try {
  ...
  const requestContacts = async () => {
    try {
      // @ts-ignore — Contacts API (Chrome Android)
  ...
  const handleContinue = () => {
    if (!name.trim()) return
    // Passe l'adresse géolocalisée à la page find via sessionStorage
    if (geoAddress) {
      sessionStorage.setItem('myAddress', geoAddress)
    }
    ...
    <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center px-5 py-12"
      ...
      <div className="w-full max-w-[420px]">
        ...
        <div className="bg-[#141414] border border-[#2A2A2A] rounded-[22px] p-8 flex flex-col gap-5">
          ...
          {/* Nom */}
          <div>
            <label className="text-[11px] tracking-[2px] uppercase text-[#555] mb-2 block">Ton prénom</label>
            <input
              type="text"
              value={name}
          ...
          {/* Email */}
          <div>
            <label className="text-[11px] tracking-[2px] uppercase text-[#555] mb-2 block">Email</label>
            <input
              type="email"
              value={email}
          ...
          <div className="flex items-center justify-between p-4 bg-[#1C1C1C] rounded-[14px] border border-[#2A2A2A]">
            <div>
              ...
              <p className="text-[#555] text-[11px] mt-0.5">
                {geoGranted === true ? (geoAddress || 'Position obtenue ✓') : geoGranted === false ? 'Refusé — saisi manuel' : 'Pour pré-remplir ton adresse'}
              </p>
            </div>
            {geoGranted === null && (
              <button
                onClick={requestGeo}
                className="px-4 py-2 bg-[#FF6B2C] text-white text-[12px] font-semibold rounded-[8px] hover:bg-[#ff7d45] transition-all"
              >
                Autoriser
              </button>
            )}
            {geoGranted === true && <span className="text-[#FF6B2C] text-[18px]">✓</span>}
            {geoGranted === false && <span className="text-[#555] text-[12px]">✕</span>}
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-[#1C1C1C] rounded-[14px] border border-[#2A2A2A]">
            <div>
              ...
              <p className="text-[#555] text-[11px] mt-0.5">
                {contactsGranted === true ? 'Accès accordé ✓' : contactsGranted === false ? 'Non disponible sur ce device' : 'Pour retrouver tes amis facilement'}
              </p>
            </div>
            {contactsGranted === null && (
              <button
                onClick={requestContacts}
                className="px-4 py-2 bg-[#2A2A2A] text-[#888] text-[12px] font-semibold rounded-[8px] hover:bg-[#333] transition-all border border-[#3A3A3A]"
              >
                Autoriser
              </button>
            )}
            {contactsGranted === true && <span className="text-[#FF6B2C] text-[18px]">✓</span>}
            {contactsGranted === false && <span className="text-[#555] text-[12px]">✕</span>}
          </div>
